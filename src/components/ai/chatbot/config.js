...
function BotChatMessage({ message }) {
  return <div className="rounded-md bg-blue-500 py-2.5 px-3 text-sm ml-2.5">
    {!message || message === "" ? "Thinking..." : <Markdown>{message}</Markdown>}
  </div>
}

const GeocoderMessage = () => {
  return (
    <div id="ai-geocoder" className="ai-geocoder text-black text-left w-full" />
  )
}
