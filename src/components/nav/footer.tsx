    ...
    <footer className="w-full bg-white">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3 md:gap-8 py-10 max-sm:max-w-sm max-sm:mx-auto gap-y-8">
          <div className="col-span-full mb-10 lg:col-span-2 lg:mb-0">
              <a href="https://pagedone.io/"  className="flex justify-center lg:justify-start">
                <Image src="/nld-logo.png" alt="Native Land Digital logo" width="38" height="60" />
              </a>
              <p className="py-8 text-sm text-gray-500 lg:max-w-xs text-center lg:text-left">Native Land Digital is a Canadian non-profit organization.</p>
          ...
          {modifiedNavItems.map(navItem => {
            return (
              <div className="lg:mx-auto text-left " key={`footer-nav-${navItem.label}`}>
                <h4 className="text-lg text-gray-900 font-medium mb-7">{navItem.label}</h4>
                <ul className="text-sm  transition-all duration-500">
                  {navItem.children.map(childNavItem => {
                    return (
                      <li className="mb-3" key={`footer-child-nav-${childNavItem.label}`}><a href={childNavItem.href} className="text-gray-600 hover:text-gray-900">{childNavItem.label}</a></li>
                    )
                  })}
                </ul>
              </div>
      ...
      <div className="py-7 border-t border-gray-200">
          <div className="flex items-center justify-center flex-col lg:justify-between lg:flex-row">
            <span className="text-sm text-gray-500">©<a className="ml-1" href="https://native-land.ca/">Native Land Digital</a> 2024, All rights reserved.</span>
